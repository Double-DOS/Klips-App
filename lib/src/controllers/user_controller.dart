...
class UserController extends ControllerMVC {
  ...
  GlobalKey<FormState> completeProfileFormKey =
      new GlobalKey(debugLabel: "completeProfile");
  GlobalKey<FormState> resetForgotPassword =
      new GlobalKey(debugLabel: "resetForgotPassword");
  GlobalKey<FormState> editVideoFormKey =
      new GlobalKey(debugLabel: "editVideoForm");
  ...
  void initState() {
    ...
    completeProfileScaffoldKey =
        new GlobalKey<ScaffoldState>(debugLabel: '_completeProfilePage');
    forgotPasswordScaffoldKey =
        new GlobalKey<ScaffoldState>(debugLabel: '_ForgotPasswordPage');
    resetForgotPasswordScaffoldKey = new GlobalKey<ScaffoldState>(
        debugLabel: '_resetForgotPasswordScaffoldPage');
    myProfileScaffoldKey =
        new GlobalKey<ScaffoldState>(debugLabel: '_myProfileScaffoldPage');
    editVideoScaffoldKey =
        new GlobalKey<ScaffoldState>(debugLabel: '_editVideoScaffoldPage');
  ...
  Future<void> getAds() {
    ...
    appId = Platform.isAndroid
        ? hashRepo.adsData.value['android_app_id']
        : hashRepo.adsData.value['ios_app_id'];
    bannerUnitId = Platform.isAndroid
        ? hashRepo.adsData.value['android_banner_app_id']
        : hashRepo.adsData.value['ios_banner_app_id'];
    screenUnitId = Platform.isAndroid
        ? hashRepo.adsData.value['android_interstitial_app_id']
        : hashRepo.adsData.value['ios_interstitial_app_id'];
    videoUnitId = Platform.isAndroid
        ? hashRepo.adsData.value['android_video_app_id']
        : hashRepo.adsData.value['ios_video_app_id'];
...
//    pref.setString("ios_email", "");
    setState(() {
      iosUuId =
          pref.getString("ios_uuid") == null ? "" : pref.getString("ios_uuid");
      iosEmail = pref.getString("ios_email") == null
          ? ""
          : pref.getString("ios_email");
    });
...
//              });
      ...
      if (e.toString().contains("Unsupported platform")) {
        userScaffoldKey?.currentState?.showSnackBar(
          Helper.toast(
              "Unsupported platform iOS version. Please try some other login method.",
              Colors.redAccent),
        );
      } else {
        userScaffoldKey?.currentState?.showSnackBar(
  ...
  loginWithFB() async {
    ...
    switch (result.status) {
      case FacebookLoginStatus.loggedIn:
        ...
        OverlayEntry loader =
            Helper.overlayLoader(userScaffoldKey?.currentContext);
...
//              });
  ...
  Future getGoogleInfo(googleSignIn) async {
    ...
    final Map<String, String> userInfo = {
      ...
      'user_dp': googleSignIn.currentUser.photoUrl != null
          ? googleSignIn.currentUser.photoUrl.replaceAll('=s96-c', '=s512-c')
          : "",
      // 'gender': gender != null ? gender : "",
  ...
  Future<void> followUnfollowUser(userId, index) async {
    ...
    userRepo.followUnfollowUser(userId).then((value) {
      ...
      if (response['status'] == 'success') {
        ...
        setState(() {
          userRepo.usersData.value.videos.elementAt(index).followText =
              response['followText'];
  ...
  Future<void> followUnfollowUserFromUserProfile(userId) async {
    ...
    userRepo.followUnfollowUser(userId).then((value) {
      ...
      if (response['status'] == 'success') {
        ...
        userRepo.userProfile.value.totalFollowers =
            response['totalFollowers'].toString();
  ...
  blockUser(userId) async {
    ...
    userRepo.blockUser(userId).then((value) async {
      ...
      if (response['status'] == 'success') {
        print(response);
        userRepo.userProfile.value.blocked =
            response['block'] == 'Block' ? 'no' : 'yes';
        userRepo.userProfile.notifyListeners();
        userScaffoldKey?.currentState?.showSnackBar(SnackBar(
      ...
      userRepo.register(userProfile).then((value) async {
        ...
        if (value != null) {
          ...
          if (response['status'] != 'success') {
            ...
            print("msg");
            print(msg);
            showAlertDialog("Error Registering User", msg,
  ...
  Future<bool> registerSocial() async {
    ...
    if (completeProfileFormKey.currentState.validate()) {
      ...
      final Map<String, String> userProfile = {
        'fname': name[0],
        'lname': name[name.length - 1],
        'email': completeProfile.email == '' || completeProfile.email == null
            ? email
        ...
        if (value != null) {
          ...
          if (response['status'] != 'success') {
            ...
            userRepo.currentUser.value =
                LoginData.fromJson(json.decode(value)['content']);
            userRepo.currentUser.value.auth = true;
            userRepo.currentUser.notifyListeners();
            connectUserSocket();
            videoRepo.homeCon.value.showFollowingPage.value = false;
            videoRepo.homeCon.value.showFollowingPage.notifyListeners();
            Navigator.of(completeProfileScaffoldKey?.currentContext)
                .pushReplacementNamed('/redirect-page', arguments: 0);
            videoRepo.homeCon.value.getVideos();
          }
        }
      ...
      userRepo.login(userProfile).then((value) async {
        ...
        if (value != null) {
          ...
          if (response['status'] != 'success') {
            ...
            print(msg);
            showAlertDialog(
                "Error Logging in", msg, userScaffoldKey.currentContext);
            return Future.value(false);
          } else {
            var content = json.decode(json.encode(response['content']));
            print("content");
            ...
            userRepo.setCurrentUser(value);
            userRepo.currentUser.value =
                LoginData.fromJson(response['content']);
            userRepo.currentUser.value.auth = true;
            userRepo.currentUser.notifyListeners();
            connectUserSocket();
            videoRepo.homeCon.value.showFollowingPage.value = false;
    ...
    if (otpFormKey.currentState.validate()) {
      ...
      userRepo.verifyOtp(data).then((value) async {
        ...
        if (response['status'] != 'success') {
          ...
          userRepo.currentUser.notifyListeners();
          connectUserSocket();
          followUnfollowUserFromUserProfile(9);
          followUnfollowUserFromUserProfile(10);
          videoRepo.homeCon.value.showFollowingPage.value = false;
          videoRepo.homeCon.value.showFollowingPage.notifyListeners();
                ...
                showSendOtp
                    ? GestureDetector(
                        onTap: () {
                          ...
                          context == otpScaffoldKey.currentContext ??
                        ...
                        child: Padding(
                          ...
                          child: Container(
                            ...
                            width: 100,
                            decoration:
                                BoxDecoration(gradient: Gradients.blush),
                            child: Center(
                              child: Row(
                                mainAxisAlignment:
    ...
    if (formKey.currentState.validate()) {
      ...
      userRepo.forgotPassword(data).then((value) async {
        ...
        if (response['status'] != 'success') {
          ...
          FocusScope.of(forgotPasswordScaffoldKey.currentContext)
              .requestFocus(FocusNode());
          forgotPasswordScaffoldKey?.currentState?.showSnackBar(
            SnackBar(
              content:
                  Text("An OTP is sent to your email please check your email."),
            ),
          );
          await Future.delayed(
            Duration(seconds: 2),
          );
          Navigator.of(forgotPasswordScaffoldKey?.currentContext)
              .pushReplacement(
            MaterialPageRoute(
    ...
    if (resetForgotPassword.currentState.validate()) {
      ...
      userRepo.updateForgotPassword(data).then((value) async {
        ...
        if (response['status'] != 'success') {
          ...
          FocusScope.of(resetForgotPasswordScaffoldKey.currentContext)
              .requestFocus(FocusNode());
          ...
          Navigator.of(resetForgotPasswordScaffoldKey?.currentContext)
              .pushReplacement(
            MaterialPageRoute(
