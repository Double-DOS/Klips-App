...
class DashboardController extends ControllerMVC {
  ...
  ValueNotifier<bool> showFollowersPage = new ValueNotifier(false);
  ValueNotifier<bool> showBannerAd = new ValueNotifier(false);
  ValueNotifier<Map<String, dynamic>> userVideoObj =
      new ValueNotifier(new Map());
  ...
  SwiperController swipeController3;
  ...
  Future<void> getAds() {
    ...
    hashRepo.getAds().then((value) {
      ...
      if (value != null) {
        ...
        hashRepo.adsData.value =
            new Map<String, dynamic>.from(json.decode(value));
        hashRepo.adsData.notifyListeners();
        appId = Platform.isAndroid
            ? response['android_app_id']
            : response['ios_app_id'];
        bannerUnitId = Platform.isAndroid
            ? response['android_banner_app_id']
            : response['ios_banner_app_id'];
        screenUnitId = Platform.isAndroid
            ? response['android_interstitial_app_id']
            : response['ios_interstitial_app_id'];
        videoUnitId = Platform.isAndroid
            ? response['android_video_app_id']
            : response['ios_video_app_id'];
  ...
  Future<void> listenForMoreUserFollowersVideos() async {
    page = page + 1;
    videoRepo.getFollowersUserVideos(page).whenComplete(() {
      loadMoreUpdateView.value = true;
      loadMoreUpdateView.notifyListeners();
    });
  }
  ...
  Future<void> likeVideo(int index) async {
    ...
    videoRepo.videosData.value.videos.elementAt(index).totalLikes =
        (!videoRepo.videosData.value.videos.elementAt(index).isLike)
    ...
    videoRepo
        .updateLike(videoRepo.videosData.value.videos.elementAt(index).videoId)
        .whenComplete(() {
    ...
    videoRepo.followingUsersVideoData.value.videos.elementAt(index).totalLikes =
        (!videoRepo.videosData.value.videos.elementAt(index).isLike)
            ...
            : videoRepo.followingUsersVideoData.value.videos
                    ...
                    .totalLikes -
                1;
    videoRepo.followingUsersVideoData.value.videos.elementAt(index).isLike =
        (videoRepo.videosData.value.videos.elementAt(index).isLike)
            ? false
            : true;

    videoRepo
        .updateLike(videoRepo.followingUsersVideoData.value.videos
            .elementAt(index)
            .videoId)
        .whenComplete(() {
      likeShowLoader.value = false;
  ...
  Future<void> submitReport(Video videoObj, context) async {
    ...
    videoRepo
        .submitReport(videoObj, selectedType, description)
        .whenComplete(() {
  ...
  Future<void> getComments(Video videoObj) async {
    ...
    final Stream<CommentData> stream =
        await commentRepo.getComments(videoObj.videoId, page);
  ...
  Future<void> addComment(int videoId, context) async {
    ...
    commentObj.time = 'now';
    commentValue = '';
    if (!showFollowingPage.value && !showFollowersPage.value) {
      videoRepo.videosData.value.videos.elementAt(swiperIndex).totalComments =
          videoRepo.videosData.value.videos
                  .elementAt(swiperIndex)
                  .totalComments +
              1;
    } else if (showFollowingPage.value) {
      videoRepo.followingUsersVideoData.value.videos
          .elementAt(swiperIndex2)
          .totalComments = videoRepo.followingUsersVideoData.value.videos
              .elementAt(swiperIndex2)
              .totalComments +
          1;
    } else {
      videoRepo.followersUsersVideoData.value.videos
          .elementAt(swiperIndex2)
          .totalComments = videoRepo.followingUsersVideoData.value.videos
              .elementAt(swiperIndex2)
              .totalComments +
  ...
  Future<void> onEditComment(index, context) async {
    ...
    commentController = new TextEditingController(
        text: videoRepo.homeCon.value.comments[index - 1].comment);
    print("editedComment.value ");
    print(editedComment.value);
  }
  ...
  VideoPlayerController videoController2(int index) {
    if (videoRepo.followingUsersVideoData.value.videos.length > 0) {
      return videoControllers2[
          videoRepo.followingUsersVideoData.value.videos.elementAt(index).url];
    }
  }

  VideoPlayerController videoController3(int index) {
    if (videoRepo.followersUsersVideoData.value.videos.length > 0) {
      return videoControllers3[
          videoRepo.followersUsersVideoData.value.videos.elementAt(index).url];
    }
  }
                      ...
                      userRepo.agreeEula().then((value) {
                        if (value) {
                          ...
                          videoRepo.homeCon.value.showFollowingPage
                              .notifyListeners();
                          videoRepo.homeCon.value.showFollowersPage.value =
                              false;
                          videoRepo.homeCon.value.showFollowersPage
                              .notifyListeners();
  ...
  Future<void> initController2(int index) async {
    try {
      ...
      videoControllers2[videoRepo.followingUsersVideoData.value.videos
          .elementAt(index)
          .url] = controller;
  ...
  Future<void> initController3(int index) async {
    try {
      var controller = await getControllerForVideo(
          videoRepo.followingUsersVideoData.value.videos.elementAt(index).url);
      print("cachedcontroller1");
      videoControllers3[videoRepo.followingUsersVideoData.value.videos
          .elementAt(index)
          .url] = controller;
      initializeVideoPlayerFutures2[videoRepo
          .followersUsersVideoData.value.videos
          .elementAt(index)
          .url] = controller.initialize();
      controller.setLooping(true);
    } catch (e) {
  ...
  void removeController2(int count) async {
    try {
      ...
      videoControllers2.remove(
          videoRepo.followingUsersVideoData.value.videos.elementAt(count));
  ...
  void removeController3(int count) async {
    try {
      await videoController3(count)?.dispose();
      videoControllers3.remove(
          videoRepo.followersUsersVideoData.value.videos.elementAt(count));
      initializeVideoPlayerFutures3.remove(
          videoRepo.followersUsersVideoData.value.videos.elementAt(count));
    } catch (e) {
      print("Catch: " + e);
    }
  ...
  void stopController3(int index) {
    videoController2(index)?.pause();
  }

  void playController3(int index) async {
    print("Play2");
    // videoControllers2.forEach((key, value) {
    //   value.pause();
    // });
  ...
  void previousVideo3(ind) async {
    if (ind < 0) {
      return;
    }
    lock = true;
    stopController3(ind + 1);

    if (ind + 2 < videoRepo.followersUsersVideoData.value.videos.length) {
      removeController3(ind + 2);
    }
    ...
    if (ind == 0) {
      ...
      initController3(ind - 1).whenComplete(() => lock = false);
    }
  }
  ...
  void nextVideo3(ind) async {
    if (ind > videoRepo.followersUsersVideoData.value.videos.length - 1) {
      return;
    }
    lock = true;
    stopController3(ind - 1);
  ...
  Future<void> followUnfollowUser(Video videoObj) async {
    ...
    if (videoRepo.homeCon.value.showFollowingPage.value) {
      ...
      if (videoRepo.followingUsersVideoData.value.videos.length == 1 &&
          videoRepo.followingUsersVideoData.value.videos
                  .elementAt(videoRepo.homeCon.value.swiperIndex2)
                  .followText ==
              "Unfollow") {
    ...
    if (videoRepo.homeCon.value.showFollowersPage.value) {
      print("followUnf1");

      if (videoRepo.followersUsersVideoData.value.videos.length == 1 &&
          videoRepo.followersUsersVideoData.value.videos
                  .elementAt(videoRepo.homeCon.value.swiperIndex3)
                  .followText ==
              "follow") {
        videoRepo.homeCon.value
            .videoController2(videoRepo.homeCon.value.swiperIndex3)
            .pause();
      }
    }
