...
ValueNotifier<VideoModel> followingUsersVideoData =
    new ValueNotifier(VideoModel());
ValueNotifier<VideoModel> followersUsersVideoData =
    new ValueNotifier(VideoModel());
...
Future<VideoModel> getVideos(page, [obj]) async {
  ...
  uri = uri.replace(queryParameters: {
    ...
    "video_id": obj != null
        ...
        : '0',
    "login_id": userRepo.currentUser.value.userId == null
        ? '0'
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (jsonData['status'] == 'success') {
        ...
        if (page > 1) {
          videosData.value.videos.addAll(
...
Future<VideoModel> getFollowersUserVideos(page) async {
  print("getFollowingUserVideos");
  Uri uri = Helper.getUri('get-videos');
  uri = uri.replace(queryParameters: {
    "page_size": '10',
...
Future<String> incVideoViews(Video videoObj) async {
  String userVideoId = userRepo.currentUser.value.userId != null
      ? userRepo.currentUser.value.userId.toString()
      : "";
...
deleteVideo(videoId) async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (jsonData['status'] == 'success') {
        userRepo.myProfile.value.userVideos
            .removeWhere((item) => item.videoId == videoId);
    ...
    if (response.statusCode == 200) {
      ...
      if (jsonData['status'] == 'success') {
        ...
        if (!homeCon.value.showFollowingPage.value) {
          videosData.value.videos
              .elementAt(homeCon.value.swiperIndex)
              .totalComments = videosData.value.videos
                  .elementAt(homeCon.value.swiperIndex)
                  .totalComments -
              1;
        } else {
          followingUsersVideoData.value.videos
              .elementAt(homeCon.value.swiperIndex2)
              .totalComments = followingUsersVideoData.value.videos
                  .elementAt(homeCon.value.swiperIndex2)
                  .totalComments -
              1;
        }
