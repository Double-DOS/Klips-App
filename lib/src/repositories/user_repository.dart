...
ValueNotifier<BlockedModel> blockedUsersData =
    new ValueNotifier(BlockedModel());
...
ValueNotifier<UserProfileModel> userProfile =
    new ValueNotifier(UserProfileModel());
ValueNotifier<UserProfileModel> myProfile =
    new ValueNotifier(UserProfileModel());
...
Future<String> register(userProfile) async {
  ...
  if (userProfile['profile_pic_file'] == null ||
    ...
    formData = FormData.fromMap({
      'fname': (userProfile['fname'] == '' || userProfile['fname'] == null)
          ? socialUserProfile.value.name.split(" ")[0]
          : userProfile['fname'],
      'lname': (userProfile['lname'] == '' || userProfile['lname'] == null)
          ? socialUserProfile.value.name.split(" ")[1]
          : userProfile['lname'],
      'email': (userProfile['email'] == '' || userProfile['email'] == null)
          ? socialUserProfile.value.email
          : userProfile['email'],
      ...
      "profile_pic_file": await MultipartFile.fromFile(
          userProfile['profile_pic_file'],
          filename: fileName),
    });
  }
  ...
  if (response.statusCode == 200) {
    ...
    if (json.decode(response.body)['status'] == "success") {
      ...
      if (json.decode(response.body)['isRecord'] == null) {
        ...
        currentUser.value =
            LoginData.fromJson(json.decode(response.body)['content']);
        currentUser.notifyListeners();
      } else {
        print("response.body else");
        socialUserProfile.value =
            LoginData.fromJson(json.decode(response.body)['content']);
...
Future<String> blockUser(userId) async {
  ...
  final response = await client.post(
    ...
    headers: headers,
    body: json.encode({
      "blocked_by": userRepo.currentUser.value.userId.toString(),
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (jsonData['status'] == 'success') {
        ...
        print(json.decode(response.body)['data']);
        if (page > 1) {
          userProfile.value.userVideos.addAll(
              UserProfileModel.fromJson(json.decode(response.body)['data'])
                  .userVideos);
        } else {
          userProfile.value =
              UserProfileModel.fromJson(json.decode(response.body)['data']);
...
Future<BlockedModel> getBlockedUsers(page) async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (jsonData['status'] == 'success') {
        if (page > 1) {
          blockedUsersData.value.users.addAll(
              BlockedModel.fromJson(json.decode(response.body)['blockList'])
                  .users);
        } else {
          blockedUsersData.value =
              BlockedModel.fromJson(json.decode(response.body)['blockList']);
        }
        usersData.notifyListeners();
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (jsonData['status'] == 'success') {
        ...
        if (page > 1) {
          myProfile.value.userVideos.addAll(
              UserProfileModel.fromJson(json.decode(response.body)['data'])
                  .userVideos);
        } else {
          myProfile.value =
